...
## PayPal integration rewrite

Commerce v1.11 includes a new **PayPal** gateway, which replaces the legacy PayPal integration. This new integration uses the modern PayPal REST v2 API and adds support for:

- **Authorizations and Captures**: Place a hold on funds and capture them later.
- **Payment Tokens (Vaulting)**: Allow customers to securely save their PayPal account for future purchases.
- **Modern Authentication**: Uses OAuth 2.0 (Client ID and Client Secret) for better security and easier setup.

The legacy NVP/SOAP-based integration is still available as **PayPal (Legacy)** for existing users and for the time being continues to work, but new shops will likely require the new PayPal Checkout integration as PayPal has stopped issuing legacy NVP credentials.

**To use the new integration, you must create a new Payment Method in Commerce and select the "PayPal" gateway.**

See the [PayPal documentation](../Payment_Methods/PayPal) for detailed setup instructions.
