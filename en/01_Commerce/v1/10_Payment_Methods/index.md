...
**Auth/capture** indicates if the payment gateway supports delayed charges. Typically, the authorization happens during checkout which places a hold on a customers' card or payment instruction, and the capture (charge) when the order ships. Depending on gateway, there may be a specific max amount or expiration date. Auth/capture flows were first added in Commerce 1.3. [Learn more about authorize & capture flows](Authorize_Capture_Flow)

Unless otherwise noted, the gateways below are included in the standard Commerce package and covered by standard support.

| Gateway                                                                                                                      | Payment options                                                                                     | On/off-site                              | SCA?       | Auth/Capture          |
|------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|------------------------------------------|------------|-----------------------|
...
| [Authorize.net](Authorize.net) (core)                                                                                        | Credit Cards (primarily USA)                                                                        | On-site (JavaScript)                     | ? (note 2) | ✓ (v1.3+)             |
...
| [Manual](Manual) (core)                                                                                                      | None; marks all transactions as successful.                                                         | On-site (server-side)                    | N/a        | ✓ (v1.3+)             |
...
| ~~[Paymill](Paymill) (core)~~ Removed in v1.3                                                                                | Paymill is no longer in business. ~~Credit Cards (primarily DE, EU)~~                               | ~~On-site (JavaScript frame)~~           | -          | -                     |
| [PayPal](PayPal) (core)                                                                                                      | Express, supporting PayPal account, credit cards                                                    | Off-site (direct)                        | ✓ (note 1) | -                     |
| [Payrexx](Payrexx) (official extension)                                                                                      | 200+ different payment options                                                                      | Off-site (hosted and/or direct)          | ✓ (note 1) | -                     |
...
| [Stripe](Stripe) (core)                                                                                                      | Credit Cards, v1.3+ also supports many local payment options                                        | On-site (v1.3+: Payment Element)         | ✓ (v1.1+)  | ✓ (v1.3+, cards only) |
...
## What about PSD2 and Strong Customer Authentication?

Please see the _SCA Ready_ column in the table above, and relevant notes, for details about the status for each of the available payment gateways.

## Need another payment provider?

With hundreds (if not thousands) of payment providers available, it is likely you'll eventually need an integration we don't have available just yet. We're always tracking interest in different gateways, so let us know which one you're missing.

To sponsor development of a payment integration, please [send us an email](mailto:support@modmore.com) with details of the payment provider, and we'd be more than happy to provide you with a cost and time estimate.

Typical gateway implementations cost in the €350-700 range to build. Complexities that can push that number higher include supporting different payment options, requiring separate implementations for SCA support, adding auth/capture support, etc. If we have other clients requesting the same gateway integration, we're happy to explore options to crowdfund or prioritise development.
